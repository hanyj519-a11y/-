...
test("proxy strips unrelated group headers before forwarding to ZX", async () => {
  ...
  try {
    const request = new Request(
      "https://canvas.example/api/proxy?url=https%3A%2F%2Fzxai.work%2Fmj-fast%2Fmj%2Fsubmit%2Fimagine",
      {
        method: "POST",
        headers: {
          Authorization: "Bearer test",
          "Content-Type": "application/json",
          "X-Comfly-Group": "Mid-Journey",
          "X-Model-Group": "Mid-Journey",
          "X-Channel-Group": "Mid-Journey",
        },
        body: '{"prompt":"test","base64Array":[]}',
      },
    );
    const response = await proxyRequest({ request });
    assert.equal(response.status, 204);
    assert.equal(forwardedHeaders.get("authorization"), "Bearer test");
    assert.equal(forwardedHeaders.get("x-comfly-group"), null);
    assert.equal(forwardedHeaders.get("x-model-group"), null);
    assert.equal(forwardedHeaders.get("x-channel-group"), null);
  } finally {
